...
`i`などでINSERTへ入り、`Esc`または固定の`jj`でNORMALへ戻ります。キー列は変更できません。
外側をクリックする、対象要素がDOMから外れる、`Ctrl+Enter`または`Cmd+Enter`を押すと、内容を同期して終了します。

## 拡張機能の有効／無効

「拡張機能を有効にする」のトグルでは、ブラウザの拡張機能管理画面とは独立してTextareaVimの動作だけを停止できます。
設定画面またはポップアップで切り替えると、開いているページにも反映されます。

## URLごとの有効範囲

URL設定は拡張機能のポップアップと通常の設定画面から変更できます。
両方の画面は同期ストレージの同じ設定を表示します。
...
URLのqueryとfragmentは判定に含めません。

無効なURLではTextareaVimを起動せず、設定が有効なURLへ戻ると次のtextareaフォーカスから起動します。
SPAのURL変更にも追従します。
